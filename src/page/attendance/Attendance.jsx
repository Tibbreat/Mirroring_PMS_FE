...
import { Button, Modal, Table, Image, Card, Row, Col, DatePicker, Radio, message } from 'antd';
...
const Attendance = () => {
            ...
            content: (
                <Table
                    dataSource={children.map((child) => ({
                        ...
                        status: attendanceStatus[child.id] || 'Chưa điểm danh',
                    }))}
                    columns={[
                        {
                            title: 'Họ và tên',
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, record) => (
                <Radio.Group
                    value={attendanceStatus[record.id] || record.status} // Default to record.status if no attendanceStatus
